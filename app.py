...
# -----------------------------------------------------------------------------
# Session-state helpers (Streamlit-safe truthiness for DataFrames)
# -----------------------------------------------------------------------------
def _returns_df_from_session() -> pd.DataFrame:
    """Return a DataFrame of returns from session_state (handles dict/DF/None safely)."""
    rd = st.session_state.get("returns_data", None)
    if rd is None:
        return pd.DataFrame()
    if isinstance(rd, pd.DataFrame):
        return rd
    # dict-like or other structures
    try:
        return pd.DataFrame(rd)
    except Exception:
        return pd.DataFrame()

def _has_returns_data(min_assets: int = 1) -> bool:
    """True if we have non-empty returns data with at least min_assets columns."""
    df = _returns_df_from_session()
    try:
        return (not df.empty) and (df.shape[1] >= int(min_assets))
    except Exception:
        return False

def _returns_nobs() -> int:
    """Number of observations (rows) in returns data."""
    df = _returns_df_from_session()
    try:
        return int(df.shape[0]) if not df.empty else 0
    except Exception:
        return 0
...
class InstitutionalVisualizer:
    ...
    def create_interactive_correlation_matrix(
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            colorbar=dict(
                title='Correlation',
                # titleside removed for Plotly compatibility (use colorbar.title if needed)
                tickformat='.2f',
                len=0.75
            ),
            xgap=1,
...
class InstitutionalCommoditiesDashboard:
    ...
    def _display_correlation_analysis(self):
        ...
        if not _has_returns_data(min_assets=2):
            return
    ...
    def _display_portfolio_performance(self, metrics: Dict, portfolio_returns: pd.Series, 
        ...
        with rolling_col1:
            ...
            rolling_sharpe = (rolling_mean - config.risk_free_rate) / rolling_std
    ...
    def _display_monte_carlo(self):
        ...
        if simulation_type == "Single Asset":
            ...
            if st.session_state.portfolio_weights and not st.session_state.returns_data.empty:
                weights = np.array(list(st.session_state.portfolio_weights.values()))
                returns_df = pd.DataFrame(st.session_state.returns_data).dropna()
            ...
            with st.spinner("Generating professional report..."):
                try:
                    ...
                    with st.expander("📊 Report Metadata"):
                        ...
                        st.metric("Data Points", _returns_nobs())
    ...
    def _run_diagnostics(self) -> Dict[str, Any]:
        ...
        if st.session_state.data_loaded and len(st.session_state.returns_data) < 100:
