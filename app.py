        ...
        for attempt in range(retries):
            try:
                ...
                if attempt == 0:
                    ...
                    df = yf.download(**download_params)
                elif attempt == 1:
...
class InstitutionalCommoditiesDashboard:
        ...
        <div class="main-header">
            ...
            <div style="margin-top: 25px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value">{len(COMMODITIES_UNIVERSE) + len(BENCHMARKS)}</div>
    ...
    def setup_sidebar(self) -> AnalysisConfiguration:
        """Setup advanced sidebar with configuration"""
        with st.sidebar:
            # Platform info
            st.markdown("""
            <div style="text-align: center; margin-bottom: 30px; padding: 20px; 
                ...
                <h2 style="margin: 0; font-size: 1.8rem;">📊 ICA v6.0</h2>
                <p style="margin: 5px 0 0 0; opacity: 0.9; font-size: 0.9rem;">
                    Institutional Commodities Analytics
                </p>
            ...
            with col1:
                start_date = st.date_input(
                    "Start Date",
                    value=st.session_state.analysis_config.start_date.date(),
            ...
            with col2:
                end_date = st.date_input(
                    "End Date",
                    value=st.session_state.analysis_config.end_date.date(),
            ...
            for (label, days), col in zip(presets, preset_cols):
                with col:
                    ...
                    for symbol, metadata in assets.items():
                        ...
                        with col1:
                            selected = st.checkbox(
                                f"{metadata.name}",
                                key=f"asset_{symbol}",
                                value=symbol in st.session_state.selected_assets,
                                help=metadata.description
                            )
                        with col2:
                            st.markdown(f'<span style="color: {metadata.color}; font-weight: bold;">{symbol}</span>',
                                      unsafe_allow_html=True)
            ...
            for symbol, info in BENCHMARKS.items():
                ...
                with col2:
                    st.markdown(f'<span style="color: {info["color"]}; font-weight: bold;">{symbol}</span>',
                              unsafe_allow_html=True)
            ...
            confidence_levels = st.multiselect(
                ...
                [0.90, 0.95, 0.99],
                default=[0.95, 0.99],
                help="Confidence levels for VaR calculations"
            )
            
            # Optimization method
            optimization_method = st.selectbox(
                "Optimization Method",
                ["Sharpe Ratio", "Minimum Variance", "Maximum Return"],
                index=0,
                help="Portfolio optimization objective"
            ).lower().replace(' ', '_')
            ...
            backtest_window = st.slider(
                "Backtest Window",
                100, 500, 250, 25,
                help="Window size for VaR backtesting"
            )
            ...
            with col1:
                if st.button(
                    ...
                    help="Load market data for selected assets"
                ):
            ...
            with col2:
                if st.button(
                    ...
                    type="secondary",
                    use_container_width=True,
                    help="Clear cache and reset application"
                ):
                    st.cache_data.clear()
            ...
            if st.session_state.data_loaded:
                ...
                benchmarks_loaded = len(st.session_state.benchmark_data)
                data_points = len(next(iter(st.session_state.returns_data.values()))) if st.session_state.returns_data else 0
                
                st.metric("Assets Loaded", assets_loaded)
                st.metric("Benchmarks", benchmarks_loaded)
                ...
                st.markdown('<br>'.join(deps_status), unsafe_allow_html=True)
    ...
    def _load_data(
        ...
        with st.spinner("🚀 Loading market data..."):
            ...
            total_symbols = len(assets) + len(benchmarks)
            loaded_symbols = 0
            ...
            for i, symbol in enumerate(assets):
                try:
                    df = self.data_manager.fetch_asset_data(symbol, start_date, end_date)
                    if not df.empty:
                        # Calculate features
                        df = self.data_manager.calculate_technical_features(df)
                        asset_data[symbol] = df
                    else:
                        failed_assets.append(symbol)
                except Exception as e:
                    failed_assets.append(symbol)
                    self._log_error(e, f"Loading asset {symbol}")
                
                loaded_symbols += 1
                progress_bar.progress(loaded_symbols / total_symbols)
                
                # Update metrics
                metrics_container.text(f"📊 Assets: {len(asset_data)} | Failed: {len(failed_assets)}")
            ...
            for i, symbol in enumerate(benchmarks):
                try:
                    df = self.data_manager.fetch_asset_data(symbol, start_date, end_date)
                    if not df.empty:
                        df = self.data_manager.calculate_technical_features(df)
                    ...
                    failed_benchmarks.append(symbol)
                    self._log_error(e, f"Loading benchmark {symbol}")
                ...
                metrics_container.text(
                    f"📊 Assets: {len(asset_data)} | Benchmarks: {len(benchmark_data)} | "
                    f"Failed: {len(failed_assets) + len(failed_benchmarks)}"
                )
            
            # Process returns data
            status_text.text("⚙️ Processing returns data...")
            returns_data = {}
            feature_data = {}
            ...
            st.session_state.selected_benchmarks = list(benchmark_data.keys())
            ...
            **✅ Successfully loaded:**
            - **{len(asset_data)}** assets
            - **{len(benchmark_data)}** benchmarks
            - **{len(returns_data)}** return series
            - **{len(next(iter(returns_data.values()))):,}** data points per series
            ...
            **📈 Coverage:**
            ...
            if failed_assets or failed_benchmarks:
                success_msg += f"\n**⚠️ Failed to load:** {len(failed_assets) + len(failed_benchmarks)} symbols"
    ...
    def run(self):
        ...
        self.analytics.risk_free_rate = config.risk_free_rate
        ...
        with tabs[0]:
            self._display_dashboard(config)
        
        with tabs[1]:
            self._display_portfolio(config)
        
        with tabs[2]:
            self._display_advanced_analytics(config)
        
        with tabs[3]:
            self._display_analytics(config)
        
        with tabs[4]:
            self._display_reports(config)
        ...
        with col2:
            ...
            <div style="text-align: center; padding: 3rem; background: var(--light); 
                ...
                <p style="color: var(--gray); font-size: 0.9rem; margin-top: 2rem;">
                    <em>Advanced features include portfolio optimization, machine learning models, 
                    risk management, and professional reporting.</em>
                </p>
            </div>
            """.format(
                sum(len(assets) for assets in COMMODITIES_UNIVERSE.values())
            ), unsafe_allow_html=True)
    ...
    def _display_portfolio(self, config: AnalysisConfiguration):
        ...
        with export_col3:
            if st.button("📋 Download Weights", use_container_width=True):
    ...
    def _display_advanced_analytics(self, config: AnalysisConfiguration):
        ...
        with adv_tabs[2]:
            self._display_risk_analysis(config)
    ...
    def _display_risk_analysis(self, config: AnalysisConfiguration):
        ...
        if selected_asset:
            ...
            with col3:
                if st.button("📊 Calculate VaR", type="primary", use_container_width=True):
...
""".format(
            ...
            len(next(iter(st.session_state.returns_data.values()))) if st.session_state.returns_data else 0
        )
