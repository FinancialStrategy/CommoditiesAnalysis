...
class InstitutionalVisualizer:
    ...
    def create_ewma_vol_ratio_chart(
        self,
        ratio: pd.Series,
        bb_mid: pd.Series,
        bb_upper: pd.Series,
        bb_lower: pd.Series,
        vol_22: Optional[pd.Series] = None,
        vol_33: Optional[pd.Series] = None,
        vol_99: Optional[pd.Series] = None,
        title: str = "EWMA Volatility Ratio with Bollinger Bands"
    ) -> go.Figure:
        """
        Create an institutional, interactive Plotly chart for:

            ratio = EWMA(22D) / (EWMA(33D) + EWMA(99D))

        and Bollinger Bands (mid/upper/lower) plotted as distinct colored lines.

        If vol_22/vol_33/vol_99 are provided, a second panel shows the component EWMA volatilities.
        """
        # Align indices safely (keep original timestamp alignment)
        r = ratio.copy()
        mid = bb_mid.reindex(r.index)
        upper = bb_upper.reindex(r.index)
        lower = bb_lower.reindex(r.index)

        show_components = (vol_22 is not None) and (vol_33 is not None) and (vol_99 is not None)

        if show_components:
            fig = make_subplots(
                rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.12,
                row_heights=[0.62, 0.38],
                subplot_titles=("EWMA Vol Ratio + Bollinger Bands", "EWMA Volatility Components (σ)")
            )
        else:
            fig = make_subplots(
                rows=1, cols=1, shared_xaxes=True, vertical_spacing=0.10,
                subplot_titles=("EWMA Vol Ratio + Bollinger Bands",)
            )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="EWMA Vol Ratio",
                line=dict(color=self.colors.get("primary", "#1a2980"), width=2.6),
                hovertemplate="Date: %{x}<br>Ratio: %{y:.6f}<extra></extra>",
            ),
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=mid.index, y=mid.values,
                mode="lines",
                name="BB Mid",
                line=dict(color=self.colors.get("gray", "#6b7280"), width=1.6, dash="dot"),
                hovertemplate="Date: %{x}<br>BB Mid: %{y:.6f}<extra></extra>",
            ),
            row=1, col=1
        )

        # --- Bollinger Upper (distinct color) ---
        fig.add_trace(
            go.Scatter(
                x=upper.index, y=upper.values,
                mode="lines",
                name="BB Upper",
                line=dict(color=self.colors.get("warning", "#f59e0b"), width=1.8),
                hovertemplate="Date: %{x}<br>BB Upper: %{y:.6f}<extra></extra>",
            ),
            row=1, col=1
        )

        # --- Bollinger Lower (distinct color) + shaded band to Upper ---
        # Add lower last so we can fill to previous (upper) trace. We'll use a light translucent fill.
        fig.add_trace(
            go.Scatter(
                x=lower.index, y=lower.values,
                mode="lines",
                name="BB Lower",
                line=dict(color=self.colors.get("danger", "#ef4444"), width=1.8),
                fill="tonexty",
                fillcolor="rgba(26, 41, 128, 0.08)",
                hovertemplate="Date: %{x}<br>BB Lower: %{y:.6f}<extra></extra>",
            ),
            row=1, col=1
        )

        # --- Component vols panel ---
        if show_components:
            v22 = vol_22.reindex(r.index)
            v33 = vol_33.reindex(r.index)
            v99 = vol_99.reindex(r.index)

            fig.add_trace(
                go.Scatter(
                    x=v22.index, y=v22.values,
                    mode="lines",
                    name="EWMA 22D σ",
                    line=dict(color=self.colors.get("secondary", "#26d0ce"), width=1.8),
                    hovertemplate="Date: %{x}<br>EWMA 22D σ: %{y:.6f}<extra></extra>",
                ),
                row=2, col=1
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=v33.index, y=v33.values,
                    mode="lines",
                    name="EWMA 33D σ",
                    line=dict(color=self.colors.get("accent", "#7c3aed"), width=1.8),
                    hovertemplate="Date: %{x}<br>EWMA 33D σ: %{y:.6f}<extra></extra>",
                ),
                row=2, col=1
            )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="EWMA 99D σ",
                    line=dict(color=self.colors.get("dark", "#1f2937"), width=1.8, dash="dash"),
                    hovertemplate="Date: %{x}<br>EWMA 99D σ: %{y:.6f}<extra></extra>",
                ),
                row=2, col=1
            )
        ...
        fig.update_layout(
            template=self.template,
            height=820 if show_components else 620,
        ...
        fig.update_yaxes(title_text="Ratio", row=1, col=1, showgrid=True)
        if show_components:
            fig.update_yaxes(title_text="σ (daily)", row=2, col=1, showgrid=True)
...
class InstitutionalCommoditiesDashboard:
    ...
    def _display_advanced_analytics(self):
        ...
        with adv_tabs[4]:
            self._display_ewma_vol_ratio(config)
    ...
    def _display_ewma_vol_ratio(self, config: 'AnalysisConfiguration'):
        """
        Display the institutional indicator:

            (EWMA 22-DAY Volatility) / (EWMA 33-DAY Volatility + EWMA 99-DAY Volatility)

        and an interactive chart with Bollinger Bands (Upper/Lower) shown as distinct colored lines.
        The calculation is performed on the selected asset's return series (daily frequency).
        """
        st.markdown("#### 📉 EWMA Volatility Ratio (22 / (33 + 99))")
        ...
        # --- Resolve available assets robustly (dict / DataFrame / Series / ndarray) ---
        assets: List[str] = []
        if isinstance(returns_data, dict):
            ...
            try:
                # Last resort: try to infer columns/keys
                if hasattr(returns_data, "keys"):
                    assets = list(returns_data.keys())
            except Exception:
                assets = []

        if not assets:
            st.warning("⚠️ No assets available in returns data")
            return
        ...
        with colC:
            bb_k = st.slider(
                ...
                value=2.0,
                step=0.5,
                key="ewma_vol_ratio_bb_k"
            )

        # --- Extract the selected return series ---
        if isinstance(returns_data, dict):
            returns = returns_data.get(selected_asset)
        elif isinstance(returns_data, pd.DataFrame):
            returns = returns_data[selected_asset]
        elif isinstance(returns_data, pd.Series):
            returns = returns_data
        else:
            # Attempt index access
            try:
                returns = returns_data[selected_asset]  # type: ignore
            except Exception:
                returns = None

        if returns is None:
            st.error("❌ Could not resolve returns series for the selected asset.")
            return
        ...
        if not isinstance(returns, pd.Series):
            try:
                ...
                st.error("❌ Returns data is not a valid time series.")
                return

        returns = returns.dropna()
        if returns.empty or len(returns) < 120:
            st.warning("⚠️ Not enough return observations to compute stable EWMA volatilities (need ~120+).")
            return

        # --- EWMA volatility computation ---
        # Note: Annualization cancels out in the ratio; we compute daily EWMA std for numerical stability.
        vol_22 = returns.ewm(span=22, adjust=False).std()
        vol_33 = returns.ewm(span=33, adjust=False).std()
        vol_99 = returns.ewm(span=99, adjust=False).std()

        denom = (vol_33 + vol_99).replace(0, np.nan)
        ratio = (vol_22 / denom).replace([np.inf, -np.inf], np.nan)

        # --- Bollinger Bands on the ratio series ---
        minp = max(10, int(bb_window * 0.6))
        bb_mid = ratio.rolling(window=bb_window, min_periods=minp).mean()
        bb_std = ratio.rolling(window=bb_window, min_periods=minp).std()
        bb_upper = bb_mid + (bb_k * bb_std)
        bb_lower = bb_mid - (bb_k * bb_std)

        # --- Headline metrics (latest) ---
        ratio_last = ratio.dropna().iloc[-1] if not ratio.dropna().empty else np.nan
        v22_last = vol_22.dropna().iloc[-1] if not vol_22.dropna().empty else np.nan
        v33_last = vol_33.dropna().iloc[-1] if not vol_33.dropna().empty else np.nan
        v99_last = vol_99.dropna().iloc[-1] if not vol_99.dropna().empty else np.nan

        m1, m2, m3, m4 = st.columns(4)
        with m1:
            st.metric("EWMA Vol Ratio", f"{ratio_last:.4f}" if np.isfinite(ratio_last) else "N/A")
        with m2:
            st.metric("EWMA 22D Vol (σ)", f"{v22_last:.6f}" if np.isfinite(v22_last) else "N/A")
        with m3:
            st.metric("EWMA 33D Vol (σ)", f"{v33_last:.6f}" if np.isfinite(v33_last) else "N/A")
        with m4:
            st.metric("EWMA 99D Vol (σ)", f"{v99_last:.6f}" if np.isfinite(v99_last) else "N/A")

        # --- Build and display the interactive chart ---
        title = f"EWMA Volatility Ratio with Bollinger Bands • {selected_asset}"
        fig = self.visualizer.create_ewma_vol_ratio_chart(
            ratio=ratio,
            bb_mid=bb_mid,
            bb_upper=bb_upper,
            bb_lower=bb_lower,
        ...
        with st.expander("📄 Show computed series (latest rows)"):
            out = pd.DataFrame({
                "ratio": ratio,
                "bb_mid": bb_mid,
                "bb_upper": bb_upper,
            ...
            st.dataframe(out.tail(300), use_container_width=True)
