...
""",
        ...
        if not HMM_AVAILABLE:
            st.warning("HMM regimes are disabled because `hmmlearn` / `scikit-learn` failed to import.")
            with st.expander("Regime dependency diagnostics (click to expand)"):
                st.write("Import error:")
                st.code(HMM_IMPORT_ERROR or "Unknown import error (no message captured).")
                st.write("Fix: pin compatible versions in `requirements.txt` (see README), then redeploy / reboot the app in Streamlit Cloud.")
